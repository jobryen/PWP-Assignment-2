...
\noindent For N = 3, this case\keyword{idea} is small enough such that we can look at the determinant directly to come up with our strategy. Since there is an odd number of elements we should consider each staring case individually:
...
\par\medskip
\par\medskip
Seeing as our method works\keyword{aha} for when either $P_0$ starts and for when $P_1$ starts, we actually already have the solution! If we are forced to move without mirroring a move, due to the outside being full, then we can place a 0 anywhere whose key column matchup doesn't already contain a 0 in the row. Since this can only happen once, its the equivalent of swapping who starts the game, just with a partially populated key columns ( this wont matter as they are already paired up). So for any $N > 4$ this game is also winning for $P_0$, independently of who starts. Let's prove this and produce an optimal play algorithm in python to test.
...
I have not \keyword{note}proved anything for $N=1$ or $N=2$, as they are trivial. The first player to move in $N=1$ will always win, and we deomonstrated every case for $N=2$ earlier, showing $P_0$ will always win
...
\noindent
\emph{The \keyword{note}intention of our algorithm is to force a linear dependency between two pairs of columns}
...
\section{Review}
I am happy with the results, \keyword{check}as they have been checked two ways, we have a proof for both $n = 3$ and $n \geq 4$, and we also have a python functino for $n \geq 4$ that always wins against a random opponent. which both show that $P_0$ will always win, independent on who starts.
...
Another idea \keyword{generalise}would be to use the rank of the matrix as the determining factor for who wins. If we were to say for $P_0$ to win, they must force $Rank(M) \leq K $. This would stop the key row method from working for small enough $K$, because even if $P_0$ formed more then 2 key rows, they could only ever reduce the rank by the number of key columns -1, this would reduce the rank by max $N/2 +1$ thus making the game fairer again. One downside to this idea is that for small N, most matrices have rank close to N. Thus, making it nearly impossible for $P_0$ to win for $K \leq N-2$.
In our original question, $K = N -1$, so this is a nice generalisation of the question
\vspace{-5pt}
\begin{figure}[h]
    \centering    \includegraphics[width=0.85\linewidth]{rank_distribution.png}
\end{figure}
...
In particular, our $N \geq 4$ algorithm that we proposed have been created and tested in python, general calculations and the image in the extension was made in python also.
