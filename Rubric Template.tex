...
\section{Entry}
\
Two people are \keyword{i know}playing a game where you take turns to add a number to an $N\times N$ matrix. One player can only place ones, and they win if the matrix has a non-zero determinant, the other player can only place 0s and wins if the determinant is 0. We don't know who starts.
From a game theory point of view we can see that the game is a finite, perfect-information, deterministic, two-player, zero-sum, partizan game but that isnt of much use to use.

We want to figure \keyword{I want}out what happens $\forall N$, does either player have a dominating strategy? Is this stragegy / winning player dependant on who starts? Will the winning player change for different values of N?

\paragraph{Vocabulary}
\begin{itemize}\keyword{Introduce}
  \item Lets call the matrix the two players are playing on, M
  \item Call the player who places 0s: Player 0, or $P_0$
  \item Call the player who places 1s: Player 1, or $P_1$
\end{itemize}

\section{Attack}
Best case would be to find some strategy or algorithm for one player that will lead to them always winning for each value of N, lets run a couple of trial cases to get a feel for the question:
I tried \keyword{specialise} the following:
\begin{itemize}
...
For N = 2: This is easily solved, as there are only two unique end states for the matrix, up to rotations and reflections. $P_0$ can force a win no matter who starts. If they start place 0 top left then in either row or column that $P_1$ didn't pick to produce a 0,0 row/column. If $P_1$ starts, $P_0$ places a 0 in opposite corner to the 1, then its impossible to create $P_1$'s winning state, thus $P_0$ wins.
\end{minipage}
\par\medskip

\noindent For N = 3, this case is still small enough such that we can look at the determinant directly to come up with our strategy. Since there is an odd number of elements we should consider each staring case individually:
...
\]
The game has now been reduces to letting each of these letters equal 1 or 0 to try force the equation to be 0. Whenever $P_1$ makes a move, populate the group with least empty elements with a 0 to eliminate it. Repeat this method and you will eliminate each group therefore forcing the determinant to equal 0.
...
We then deploy the same technique, paying special care to the two-term group $ie$: if $P_1$ plays in either $i$ or $e$, you must immediately respond by placing a $0$ in the other. This will also force the determinant to equal 0, we will prove this later.


When \keyword{check}we try simulating two players playing randomly, all we need to do is generate a random matrix with 8 1s and 8 2s, to simulate the final state of the game. The order of who starts does not matter as each will result in the same number of 1s and 0s as the matrix has an even number of elements. This gives, $P_1$ wins between (55-60) percent of the time, but this isn't a very good metric, maybe we can find this deterministically by calculating all game states, and finding the ratio that player 0 wins. The number of possible end matrices will be $16\choose8$ = 12870 as the game state can just be reduces to a string of 8 1s and 8 0s uniquely arranged. this number includes rotations and reflections but that wont effect the ratio of winning/loosing games. another quick python code gives us that 58.6 percent of end games are winning for $P_0$. This gets my hopes up for finding an algorithm that will force $P_0$ to win for larger $N$!
...
\vspace{10pt}
I have found a method for \(n=4\), where you simply mirror the other person's move.
If \(P_0\) starts, place a random element, then continue mirroring until eventually they must
place the final element that mirrors your original \(0\). This seems to always produce a
non-invertible matrix, and so player \(0\) wins. This seems easier to generalise to higher dimensions of \(n\). Lets try!
...
For N > 4: lets try the \keyword{Stuck}same method, where we mirror the entries. there wont be an issue with this for even N, but what do we do if N is odd, there will be a central row that we cannot mirror. what do we do?
Lets try just adding an element th somewhere in the middle row whenever they do. But wait, there are an odd number of Columns in the row, so eventually there will be no space to mirror their move.

Lets take a step back and look into the maths behind our method our method for $N = 4$ and see if we can adapt it for larger N
...
Let $n\ge 4$. Player $0$ wins iff the final\keyword{Notation} matrix $M\in\{0,1\}^{n\times n}$ satisfies $\det(M)=0$, Player $0$ has a winning strategy given by our algorithm so following this strategy forces $\det(A)=0$. We need to prove this.
For each column $j\in\{1,2,\dots,n\}$ define the \emph{key pairs}
\[
P_{12}(j):=\bigl((j,1),(j,2)\bigr),\qquad P_{34}(j):=\bigl((j,3),(j,4)\bigr).
\]
During play, each cell is in $\{0,1,\ast\}$, where $\ast$ denotes an empty cell. We call the two cells of a key pair partners.
...
{Any blank cells outside?};

\node[block, right=of d2] (playout)
{\textbf{Play outside.} Write $0$ in any available cell \\ outside.};

\node[decision, below=of d2] (d3)
{Outside full, Any open pair\\$(0,\ast)$ or $(\ast,0)$ in key columns?};

\node[block, right=of d3] (makeopen)
{\textbf{Create open pair.} Choose any empty key pair that is $(\ast,\ast)$ and write $0$ in one cell.};

\node[block, below=of d3] (broken)
{\textbf{Mistake made, no optimal move.}\\
A mistake has been made previously, just place a zero anywhere but the algorithm may no longer work.};
...
\draw[line] (d2.south) -- node[left]{No} (d3.north);

\draw[line] (d3.east) -- node[above]{No} (makeopen.west);
\draw[line] (d3.south) -- node[left]{Yes} (broken.north);


\end{tikzpicture}}
\end{figure}
\end{claim}
The intention of our algorithm is to force a linear dependants between two pairs of columns
...
We claim that $\forall j\in\{1,2,\dots,n\}$, if
\[
(m_{1j},m_{2j})\notin\{(0,0),(1,1)\}
\quad\text{and}\quad
(m_{3j},m_{4j})\notin\{(0,0),(1,1)\},
...
\]
hence $m_{1j}+m_{2j}=1=m_{3j}+m_{4j}$ for all $j$, i.e.
\[
...
Assume Player 1 places a 0 in an empty key pair, this implies the following. There are no other partially filled pairs and the outside space is full. let K be the number of remaining empty pairs, k >= 0. Then there are 2K + 1 remailing moves. If you place a 1 in an empty pair (the only option appart from filling (0,*)) then $P_0$ fills its partner cell. This repeats k times untill the only move you have left is to fill (0,*). Therefore no key pair will ever become (0,0)
...
\section{Review}
Im happy with the results, \keyword{check}as they have been checked two ways, we have a proof and a python algorithm for both n = 3 and n >= 4, which both show that $P_0$ will always win, independent on who starts.

We can \keyword{extension}make the question more interesting by making it more difficult for player 0 to win, so we need to introduce a method for $P_1$ to escape the forced linear dependance. Lets expand $P_1$'s move-set to $\{-1,1\}$. Hopefully this will make the game more fair.

Consider N = 2,3 since $P_0$'s method is to force every element of the determinant to be zero, allowing $P_1$ to chose any range of real numbers would not help. Where this becomes useful for $P_1$ is $N\ge4$, as thats where $P_0$'s strategy is broken. I tried many iterations of this game but was unable to find any sort of dominating stragety for either, thus becoming a more fair game.

Another idea would be to use the rank of the matrix as the determining factor for who wins. If we said for $P_0$ to win, they must force $Rank(N) \leq N/2$. eg for N = 4, $P_0$ would have to construct two linear dependencies for the columns.
This would also stop the key row method from working, because even if $P_0$ formed more then 2 key rows, they could only ever reduce the rank by the number of key columns -1, this would reduce the rank by max $N/2 +1$ this would make the game fairer again.
one downside to this idea is that for small N, most matrices have rank close to N. we would need to chose $K$, $Rank(N) \leq N/2$ with K
...
In particular, all algorithms that we proposed have been created and tested in python.
